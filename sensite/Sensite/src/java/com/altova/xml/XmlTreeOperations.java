...
public class XmlTreeOperations
{
	...
	// unused by mapforce or wrapper only code
    ...
    public static void findDeclarationsForNamespace(Node node, String uri, java.util.List<Node> declarations)
    {
    ...
    public static void setValue(Node node, MemberInfo member, String value)
    {
        ...
        if (member.getLocalName() != null)
        {
            ...
            {
				java.util.ArrayList<Node> declarations = new java.util.ArrayList<Node>();
                findDeclarationsForNamespace(node, member.getNamespaceURI(), declarations);
                if( !declarations.isEmpty() )
                    prefix = ((Node) declarations.get(0)).getLocalName() + ":";
            }

            org.w3c.dom.Attr attr = element.getOwnerDocument().createAttributeNS(member.getNamespaceURI(), prefix + member.getLocalName());
    ...
    public static java.math.BigInteger castToBigInteger(Node node, MemberInfo member)
    {
        return com.altova.CoreTypes.castToBigInteger( getTextContent(node) );
    }

    public static java.math.BigDecimal castToBigDecimal(Node node, MemberInfo member)
    {
