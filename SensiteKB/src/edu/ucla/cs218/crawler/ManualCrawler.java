...
public class ManualCrawler extends WebCrawler {
        ...
        if (SPLIT_ON_PUNCTIONATION_MARKS) {
            ...
            for (int k = 1; k < 2; k++) {
                ...
                for (String line : lines) {
                    ...
                    // With Stopwords
                    BasicDBObject mShingle = new BasicDBObject("phenomena", phenomenon)
                        .append("k", k)
                        .append("stopwords", 1);
                    BasicDBList mSets = new BasicDBList();
                    for (String aShingle : freqCountStopwords.keySet()) {
                        BasicDBObject shingleSet = new BasicDBObject("set", aShingle)
                                .append("count", freqCountStopwords.get(aShingle));
                        mSets.add(shingleSet);
                    }
                    mShingle.append("sets", mSets);
                    db3.getCollection("ManualShingle").insert(mShingle);
                    
                    // Without Stopwords
                    BasicDBObject mShingleWOS = new BasicDBObject("phenomena", phenomenon)
                        .append("k", k)
                        .append("stopwords", 0);
                    BasicDBList mSetsWOS = new BasicDBList();
                    for (String aShingle : freqCountWithoutStopwords.keySet()) {
                        BasicDBObject shingleSet = new BasicDBObject("set", aShingle)
                                .append("count", freqCountWithoutStopwords.get(aShingle));
                        mSetsWOS.add(shingleSet);
                    }
                    mShingleWOS.append("sets", mSetsWOS);
                    db3.getCollection("ManualShingle").insert(mShingleWOS);
                }
            }
        }
    }
    ...
    public static void main(String args[]) throws IOException {
        ...
        crawl("A clock is an instrument to indicate, keep, and co-ordinate time. A sundial is a device that measures the time of day by the position of the sun. A water clock or clepsydra is any timepiece in which time is measured by the regulated flow of liquid into (inflow type) or out from (outflow type) a vessel where the amount is then measured. hourglass measures time by using carefully calculated amounts of sand. The hourglass uses the flow of sand to measure the flow of time.The most accurate timekeeping devices are atomic clocks. Clock is an object that shows the time.", "time");
        
        //crawl("is a switching device activated by rainfall. There are two main applications for", "rain");
        //crawl("a rain gauge is a type of instrument used by meteorologists and hydrologists to gather and measure the amount of liquid precipitation over a set period of time", "rain");
    }
}
